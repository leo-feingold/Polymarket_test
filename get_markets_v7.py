...
def calc_prices(df, client):
    ...
    for index, row in df.iterrows():
        try:
            ...
            yes_book = client.get_order_book(str(row["yes_clob_id"]))
            no_book = client.get_order_book(str(row["no_clob_id"]))
            ...
            df.at[index, "yes_price"] = min_yes_ask
            df.at[index, "yes_size"] = min_yes_ask_size
            df.at[index, "no_price"] = min_no_ask
            df.at[index, "no_size"] = min_no_ask_size

        except Exception as e:
            print(f"Error fetching order book for row {index}: {e}")
...
def main():
    ...
    new_df = calc_prices(df, client)

    new_df.to_csv("polymarket_markets5.csv")
